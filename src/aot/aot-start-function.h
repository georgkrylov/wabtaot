...
class AOTStartFunctionBuilder : public TR::MethodBuilder {
 public:
    AOTStartFunctionBuilder(const char* start_fn_name, AOTTypeDictionary* types)
        ...
        start_fn_name_(start_fn_name)
    {
      ...
      pValueType_ = types_->PointerTo(valueType_);
    ...
    bool buildIL() override {
      ...
      auto* stack_top_value  = IndexAt(types_->LookupStruct("ValueStack"), stack, ConstInt32(0));
      auto* stack_base_value = IndexAt(types_->LookupStruct("ValueStack"), stack, ConstInt32(0));
