...
TR::RelocationRecord* WASM::RelocationRecord::create(TR::RelocationRecord *storage, TR::RelocationRuntime *reloRuntime, TR::RelocationTarget *reloTarget, TR::RelocationRecordBinaryTemplate *record){
	...
	case TR_DataAddress : reloRecord = reinterpret_cast<TR::RelocationRecord*>( new (storage) WASM::RelocationRecordDataAddress(reloRuntime,record));
        break;
        default:
...
WASM::RelocationRecordMethodCallAddress::preparePrivateData(TR::RelocationRuntime *reloRuntime, TR::RelocationTarget *reloTarget){
   WASM::RelocationRecordWithOffsetPrivateData *reloPrivateData = &(privateData()->offset)  ;
   uint8_t *baseLocation = 0;
   uint8_t *callTargetAddress = computeTargetMethodAddress(reloRuntime, reloTarget, baseLocation);
   reloPrivateData->offset = (callTargetAddress - baseLocation);
}
...
WASM::RelocationRecordDataAddress::RelocationRecordDataAddress():WASM::RelocationRecord(){
    std::cout<<"Hello from WASMRelocationRecordTwo, with kind"<<std::endl;
}

WASM::RelocationRecordDataAddress::RelocationRecordDataAddress(TR::RelocationRuntime *reloRuntime, TR::RelocationRecordBinaryTemplate *record):WASM::RelocationRecord(reloRuntime, record){
    std::cout<<"Hello from WASMRelocationRecordTwo, with kind"<<std::endl;
}
