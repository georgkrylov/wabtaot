...
int wabt::aot::AOTManager::CheckDependenciesCompiled(wabt::interp::Environment *env, wabt::Index ind, wabt::interp::DefinedFunc *func, interp::Thread *t)
   ...
   if (header != NULL)
       ...
       */
      if (header->dependenciesCompiled == 1 && something_was_loaded == false)
         {
         shouldFailCheck = 0;
         }
      else if (header->dependenciesCompiled == 0 || something_was_loaded == true)
         {
         for (unsigned int i = 0; i < dependenciesMaxSize; i++)
            ...
            {
               bool loadingResult = AOTLoadAFunction(env, dependenciesArray[i], t);
            }
            else
            {
               visited_this_traversal.emplace_back(dependenciesArray[i]);
...
bool wabt::aot::AOTManager::AOTLoadAFunction(wabt::interp::Environment *env, wabt::Index ind, wabt::interp::Thread *t)
   ...
   if (!func->is_loaded)
      ...
      if (function != nullptr) /* was able to find a compiled code for the function */
         {
         ...
         if (CheckDependenciesCompiled(env, ind, fn, t) != 0) /* This if statement could possibly contain compilation strategies???*/
             ...
             * */
            while (visited_this_traversal.empty() == false)
               {
               visited_this_traversal.erase(std::remove(visited_this_traversal.begin(), visited_this_traversal.end(), ind));
               if (!visited_this_traversal.empty())
                  {
                  int next_index = visited_this_traversal.back();
                  AOTLoadAFunction(env, next_index, t);
                  }
               }
...
void *wabt::aot::AOTManager::AOTCompileAFunction(wabt::interp::Environment *env, wabt::Index ind, wabt::interp::DefinedFunc *fn, wabt::interp::Thread *t)
   ...
   if (!func->is_loaded)
      {
      /** This list is to prevent visiting the functions multiple times trying to load them in
      ...
      if (_loadStoreDriver->getRegisteredAOTMethodHeader(func->dbg_name_.c_str()) == NULL)
         {
